import { createRequestId, errorResponse, jsonResponse } from "@/lib/http/json";
import {
  ActorAuthenticationError,
  authenticateActorRequest,
} from "@/modules/actors/actor-guards";
import { armActor, isActorKey, isUuid } from "@/modules/actors/barrier";
import { db } from "@/db/client";
import { runActors } from "@/db/schema";
import { and, eq } from "drizzle-orm";

type RouteContext = {
  params: Promise<{ runId: string; actorKey: string }>;
};

function authStatus(error: ActorAuthenticationError): number {
  return error.code === "actor_scope_mismatch" ? 403 : 401;
}

export async function POST(
  request: Request,
  context: RouteContext,
): Promise<Response> {
  const requestId = createRequestId();
  const { runId, actorKey } = await context.params;

  if (!isUuid(runId) || !isActorKey(actorKey)) {
    return errorResponse(
      requestId,
      "invalid_actor_route",
      "The run or actor route is invalid.",
      400,
    );
  }

  try {
    authenticateActorRequest(request, { runId, actorKey });
  } catch (error) {
    if (error instanceof ActorAuthenticationError) {
      return errorResponse(requestId, error.code, error.message, authStatus(error));
    }
    throw error;
  }

  let actor: { id: string } | undefined;

  try {
    [actor] = await db
      .select({ id: runActors.id })
      .from(runActors)
      .where(and(eq(runActors.runId, runId), eq(runActors.actorKey, actorKey)))
      .limit(1);
  } catch (error) {
    console.error(
      JSON.stringify({
        event: "actor_lookup_failed",
        requestId,
        reason: error instanceof Error ? error.name : "unknown_error",
      }),
    );

    return errorResponse(
      requestId,
      "actor_lookup_failed",
      "The actor could not be loaded.",
      503,
    );
  }

  if (!actor) {
    return errorResponse(
      requestId,
      "actor_not_found",
      "The actor does not belong to this run.",
      404,
    );
  }

  try {
    const result = await armActor({ runId, actorKey, actorId: actor.id });

    if (!result) {
      return errorResponse(
        requestId,
        "barrier_not_found",
        "The run barrier does not exist.",
        404,
      );
    }

    return jsonResponse(requestId, result, 200);
  } catch (error) {
    console.error(
      JSON.stringify({
        event: "actor_arm_failed",
        requestId,
        reason: error instanceof Error ? error.name : "unknown_error",
      }),
    );

    return errorResponse(
      requestId,
      "actor_arm_failed",
      "The actor could not be armed.",
      500,
    );
  }
}
